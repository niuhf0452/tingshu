...
执行命令在 [项目根 README.md](../README.md) "Seed the voice library" 章节。

**自然语言描述模式**（VoiceDesign 对描述敏感，全库统一走"平和自然"基调）：

整个 catalog 不再区分"旁白类"与"角色类"两套写法 —— 所有 tag（narrator / 角色 / 童声）都按有声书自然朗读的风格来写：

- **气质 + 音色描述**先行：年龄、声线特征（清亮 / 低沉 / 温润 / 带气声）、音区（中音 / 中低音）。
- **习惯性的语速 / 语调**而非演绎指令：写"语速从容"、"句尾自然下沉"、"语调起伏自然"，不写"重音夸张外放"、"句尾上挑制造 XX 感"。
- **场景比喻**用日常画面而非影视片段："像朋友轻声讲话"、"像在安静的房间里说话"，少用 "宫斗剧妃嫔" / "霸道总裁" 这类带表演预设的影视锚。
- **必有 dampener**："不刻意"、"不夸张"、"自然"、"克制" —— 反向收住任何被前文激活的戏剧化语气。

> 历史上 character 类用过"戏剧化 + persona-anchored"的写法，刻意要求"比 narrator 类更多表演感"。听感反馈是声音过度表演（"像在朗诵"、"像在演戏"），且 runtime tone instruct 难以再调控 —— 参考音色已锁死在一个夸张的 baseline 上。2026-05 起全部回归平和基调，persona 仅靠音色 / 年龄 / 习惯语速来区分。

**参考文本（ref_text）也保持平和**：64 条独立的短句声明文（每条 12-18 字 + 句号），无引号、无感叹、无戏剧性短语 —— 见 `scripts/generate_voicedesign_voices.py:REF_TEXT_FOR_TAG`。年龄差异在场景上软保留（child/teen 用学校 / 玩具 / 玩耍场景），但情绪一律中性。

> 早期版本曾按 narrator / adult / teen / child 4 档共享长 ref_text，结果 51 条 voice 全部把参考文本尾巴泄露到生成内容里（"…只有强者，和弱者…"）。零样本克隆把 `[ref_text_tokens] + [target_text_tokens]` 拼接，长 / 戏剧化 / 共享的 ref_text 形成强 attention attractor，让模型在边界处拽不回来。换成短、中性、per-tag 唯一句之后泄露消失。
